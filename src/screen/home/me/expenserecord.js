...
class ExpenseRecord extends PureComponent {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    ...
    var API_URL = CONFIG.ENDPOINT_OUR + "/api/expenserecord";
    console.log("expenserecord.js", "componentWillMount", API_URL);
    ...
    .then(res => {
        if (res.success == "success") {
          this.setState({
            isRefreshing: false,
            dataSource: this.state.dataSource.cloneWithRows(res.expenselist),
            record: res.expenselist
          });
        } else {
